---
title: 'Get a Portable RAG Stack & Vector Database with Verba and Flox'
altTitle: 'Get a Portable RAG Stack & Vector Database with Verba and Flox'
...
## Setting up your RAG environment with Verba & Flox
...
Let’s walk through the sections of this environment and explore how it works, so you can understand how to easily customize it to suit your needs.

First, run `flox edit` to view and edit its manifest. If you’ve already configured an `EDITOR` environment variable, running `flox edit` opens its manifest in your preferred editor. (If you haven’t done so, `flox edit` defaults to `nano`.) To edit this file manually, just open up [the manifest.toml file](https://flox.dev/blog/mastering-hooks-and-profiles-for-reproducible-flox-environments/) that lives in `./.flox/env/`.
...
## Configuring your RAG stack with Flox
...
To get started with Verba, fire up your Web browser and point it to port 8000 on your localhost. Click the "Custom" button...

![](/images/blog/verba-0.png)

...and accept the defaults in the configuration menu:

![](/images/blog/verba-00.png)

You can use the drop-down menu to configure your RAG pattern’s settings, add documents, search through documents, and perform other tasks. (If you’re using Verba on a large enough screen, this menu is replaced by descriptive clickable buttons.)
...
Clicking the `Config` tab in the right-hand side of the `Import Data` wizard lets you specify which embedding (for extracting vector embeddings) and generator (for LLM prompting) you want to use. Ollama is configured by default.
...
![](/images/blog/verba-5a.png)

And here's the Verba context window, which shows the document the model uses to generate the response:

![](/images/blog/verba-5b.png)
